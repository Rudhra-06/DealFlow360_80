...
#### Database Migrations (Alembic — Phase 1 — Part 3)
Alembic controls all database schema evolutions. Run migration commands from the `backend/` directory:

- **Create Migration** (after modifying SQLAlchemy models in `app/models/`):
  ```bash
...
- **Rollback One Revision**:
  ...
  ```
> [!IMPORTANT]
> Always manually inspect autogenerated migration files in `backend/alembic/versions/` before applying (`alembic upgrade head`). Detailed migration guide available at [`docs/DATABASE_MIGRATIONS.md`](file:///c:/Users/Nami..%21%21/DealFlow360_80/docs/DATABASE_MIGRATIONS.md).
