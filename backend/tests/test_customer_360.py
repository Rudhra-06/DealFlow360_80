...
async def test_customer_360_full_aggregation(db_session: AsyncSession):
    ...
    rep = User(email="c360_rep@test.com", password_hash="hash", full_name="C360 Rep", is_active=True)
    ...
    cust = Customer(customer_code="CUST-360-01", company_name="Omega 360 Corp", tier_id=tier.id, assigned_sales_rep_id=rep.id)
    db_session.add(cust)
    await db_session.flush()

    q = Quotation(
        quotation_number="QT-360-001",
        customer_id=cust.id,
        sales_rep_id=rep.id,
        status="CUSTOMER_CONFIRMED",
        currency="USD",
        net_total=Decimal("25000.00"),
        effective_discount_pct=Decimal("12.00"),
    ...
    sub = Subscription(subscription_number="SUB-360-001", customer_id=cust.id, status="ACTIVE", currency="USD", monthly_recurring_revenue=Decimal("1500.00"), billing_frequency="MONTHLY", start_date=datetime.now(timezone.utc))
    db_session.add(sub)
...
async def test_customer_360_security_isolation(db_session: AsyncSession):
    ...
    rep1 = User(email="rep1_360@test.com", password_hash="hash", full_name="Rep One", is_active=True)
    rep1.roles.append(role_rep)

    rep2 = User(email="rep2_360@test.com", password_hash="hash", full_name="Rep Two", is_active=True)
    rep2.roles.append(role_rep)

    db_session.add_all([rep1, rep2])
    await db_session.flush()

    cust1 = Customer(customer_code="CUST-360-REP1", company_name="Rep1 Customer", assigned_sales_rep_id=rep1.id)
